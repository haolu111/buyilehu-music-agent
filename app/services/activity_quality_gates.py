...
from app.services.music_practice_classification import activity_practice_profile
...
def _practice_first_gate(activity: dict[str, Any]) -> dict[str, str]:
    activity_id = str(activity.get("activity_id") or "").strip()
    explicit_role = str(activity.get("pedagogical_role") or "").strip()
    valid_roles = {
        "core_practice",
    ...
    try:
        role = str(activity_practice_profile(activity_id)["role"])
    except ValueError:
        role = explicit_role
    ...
    if role not in valid_roles:
        return _gate(
            ...
            "实践优先",
            "fail",
            "活动必须声明有效课堂角色：core_practice、practice_with_evidence、listening_evidence 或 closure_assessment。",
        )

    if role in {"listening_evidence", "closure_assessment"}:
        return _gate(
            "practice_first_gate",
            "实践优先",
            "pass",
            "该活动仅用于听赏证据、选择整理或课末反馈，不能替代课堂主活动；教师必须把证据带回真实音乐实践。",
        )

    alignment = activity.get("education_alignment")
    alignment = alignment if isinstance(alignment, dict) else {}
    practices = _string_set(activity.get("student_music_behaviors")) or _string_set(alignment.get("student_practices"))
    core_practices = {"tap", "move", "sing", "read", "draw", "play", "create", "revise", "perform", "cooperate"}
    matched = practices.intersection(core_practices)
    if not matched:
        return _gate(
            "practice_first_gate",
            "实践优先",
            "fail",
            "课堂主活动必须让学生拍、动、唱、读、画、奏、创、修订、展示或合作；仅听、选、配、排、填不能作为主活动。",
        )
    return _gate(
        "practice_first_gate",
        "实践优先",
        "pass",
        f"课堂主活动包含真实音乐实践：{', '.join(sorted(matched))}。网页记录不能替代教师对真实表现的判断。",
    )
