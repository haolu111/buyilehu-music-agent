...
from app.services.game_template_registry import build_game_instance
...
def build_primary_music_game_runtime(
    *,
    ...
    if composition.get("selected_activity_id") == "meter_body_movement":
...
def _phrase_singing_runtime(*, composition: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    practice_variant = "phrase_loop_singing" if activity_id == "phrase_loop_singing" else "difficult_phrase_repair" if difficult_phrase or breath_points else "phrase_loop_singing"
    ...
    state = {
        "workflow": {
            ...
            "gameplay_blueprint": {
                ...
                "scene_goal": "先听短旋律，再自由画出自己听到的高低走向，并回到唱回。",
                "game_genre": "freehand_melody_contour_trace",
            },
        },
        "instance": {
            ...
            "student_task": {
                ...
                "do": "做什么：在空白画板上自由画出你听到的旋律走向。",
                "pass": "怎样完成：教师结合描画、说明和唱回确认旋律走向。",
            },
        },
        ...
        "config": {
            ...
            "show_contour_hint": False,
            "record_export": "melody_contour_free_draw_record_v2",
            "education_alignment": deepcopy(composition["education_alignment"]),
            "teacher_explanation": deepcopy(composition["teacher_explanation"]),
        },
        "student_task_copy": {
...
def _strong_weak_beat_runtime(*, composition: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    state = {
        "workflow": {
            ...
            "education_alignment": deepcopy(composition["education_alignment"]),
            "gameplay_blueprint": {
                "student_facing_name": "强弱拍律动",
                "prompt": composition["teacher_explanation"]["why_this_game"],
                "operation_type": "strong_weak_beat_body_movement",
                "scene_goal": "先听稳定拍和强弱关系，再用大动作、小动作表现拍号强弱规律。",
                "game_genre": "guided_meter_body_movement",
            },
        },
        "instance": {
            "template_label": "强弱拍律动",
            "student_task": {
        ...
        "template_id": "primary_strong_weak_beat",
        "age_ui_profile": _age_profile(composition.get("difficulty", {}).get("grade_band", "")),
        "config": {
            "activity_id": activity_id,
            ...
            "movement_actions": movement_actions,
            "record_export": "meter_body_movement_record_v1",
            "education_alignment": deepcopy(composition["education_alignment"]),
            "teacher_explanation": deepcopy(composition["teacher_explanation"]),
        },
        "student_task_copy": {
        ...
        "music_reason_prompts": {
            ...
            "success": "强弱拍稳定：回到歌曲或律动片段中用身体动作表现。",
        },
        "result_transfer_prompt": "回到教材音乐片段，用拍手、拍腿等身体律动表现二拍子或三拍子的强弱规律。",
    }
    return {
        "runtime_builder_version": RUNTIME_BUILDER_VERSION,
        "student_game_state": state,
...
def _graphic_score_runtime(*, composition: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    bpm = int(available.get("bpm") or composition.get("difficulty", {}).get("bpm") or 88)
    creation_mode = _graphic_score_creation_mode(request, available)
    state = {
        "workflow": {
            "workflow_kind": "primary_activity_game",
            "composition_spec": _public_composition_summary(composition),
        ...
        "config": {
            ...
            "rhythm_card_ids": _graphic_card_ids(
                available,
                direct_key="rhythm_card_ids",
                material_key="rhythm_pattern",
                fallback=["quarter", "eighth_pair", "sixteenth_four", "rest"],
            ),
            "pitch_card_ids": _graphic_card_ids(
                available,
                direct_key="pitch_card_ids",
                material_key="solfege_set",
                fallback=["do", "re", "mi", "fa", "sol", "la", "ti"],
            ),
...
def _graphic_score_creation_mode(request: dict[str, Any], available: dict[str, Any]) -> str:
    explicit = available.get("creation_mode") or request.get("creation_mode")
...
def _graphic_card_ids(
    ...
    *,
    ...
    value = available.get(direct_key) or available.get(material_key)
    if isinstance(value, str):
        return [value]
    if isinstance(value, list):
        ids = [
            str(item.get("id") or item.get("pattern_id") or item.get("pitch_id"))
            if isinstance(item, dict)
            else str(item)
            for item in value
        ]
        normalized = [item for item in ids if item and item != "None"]
        if normalized:
            return normalized
