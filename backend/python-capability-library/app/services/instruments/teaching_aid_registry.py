...
from app.services.instruments.teaching_aid_spec import validate_teaching_aid_spec
...
TEACHING_AID_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "melody_contour_board": _with_asset_policy({
        ...
        "runtime": _runtime(
            ...
            ["contour_step", "gesture", "timestamp_ms", "status"],
            ["学生能先听旋律，再用手势跟踪上行、下行和平稳，并回到唱回。"],
    ...
    "graphic_score_cards": _with_asset_policy({
        ...
        "grade_bands": ["middle_primary", "upper_primary"],
        "material_entities": ["graphic_symbol_meanings", "meter"],
        "components": ["graphic_score_canvas", "audio_player"],
        ...
        "acceptance": deepcopy(_DEFAULT_ACCEPTANCE),
        "runtime": _runtime(
            ["GraphicScoreActivity", "GraphicScoreBoard", "AudioPlayer"],
            ["graphic_score_create", "xylophone_creation"],
            ["shape_id", "music_meaning", "timestamp_ms"],
            ["学生能用图形表示节奏、力度或旋律线并说明。"],
            extra_quality_gates=["symbol_meaning_bound", "playback_ready"],
        ),
    }),
