...
function componentDisplayName(name, componentKey) {
    var candidates = [name, componentKey].filter(function (value) { return Boolean(value === null || value === void 0 ? void 0 : value.trim()); });
    for (var _i = 0, candidates_1 = candidates; _i < candidates_1.length; _i++) {
        var candidate = candidates_1[_i];
        var label = componentLabels[normalized(candidate)];
        if (label)
            return label;
    }
    var fallback = candidates[0] || '互动活动';
    return /[\u3400-\u9fff]/.test(fallback) ? fallback : '互动活动';
}
function nodeTypeDisplayName(type) {
    if (!type)
        return '课堂活动';
