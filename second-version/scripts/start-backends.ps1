...
if ($null -eq [Environment]::GetEnvironmentVariable("DB_PASSWORD", "Process")) {
    throw "DB_PASSWORD must be set in this terminal; it is intentionally not stored in this repository."
}
$env:DB_NAME = if ($env:DB_NAME) { $env:DB_NAME } else { "buyilehu_music_agent_v2" }
